...
const VerifyRegistrationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8081/auth/verify-registration', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-white">
      ...
      <div className="px-8 py-6 mt-4 text-left bg-white shadow-lg rounded-lg w-96">
        <h3 className="text-2xl font-bold text-center mb-6">Verifikasi Pendaftaran</h3>
        <form onSubmit={handleSubmit}>
          <div className="mt-4">
            ...
            <div className="mb-4">
              <label className="block text-lg font-medium" htmlFor="otp">Kode OTP</label>
              <input
                type="text"
                id="otp"
                value={token}
                ...
                className="w-full px-4 py-2 mt-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-500"
              />
            </div>
            {otp && (
